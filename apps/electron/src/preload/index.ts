...
import type {
  ...
  BotSidecarState,
  EnsureBotSidecarSessionInput,
  CloseBotSidecarInput,
  OpenBotSidecarInput,
  PublishBotConfigRevisionInput,
  SaveBotMemoryCandidateInput,
  ...
  DownloadCollaborationArtifactResult,
  ImportCollaborationWorkspaceResponse,
...
const electronAPI = {
  ...
  stopAgent: (sessionId: string) =>
    ipcRenderer.invoke(AGENT_IPC_CHANNELS.STOP_AGENT, sessionId),
   ...
   */
  readGitHeadFile: (input: {
    sessionId: string;
    path: string;
    bases?: string[];
  }) =>
    ipcRenderer.invoke(AGENT_IPC_CHANNELS.READ_GIT_HEAD_FILE, input) as Promise<
      string | null
    >,
  /** 销毁会话（杀进程 + 删元数据/JSONL） */
  ...
  listMoaPresets: () =>
    ipcRenderer.invoke(AGENT_IPC_CHANNELS.LIST_MOA_PRESETS) as Promise<
      MoAPreset[]
    >,
  /**
  ...
  saveMoaPresets: (presets: MoAPreset[]) =>
    ipcRenderer.invoke(AGENT_IPC_CHANNELS.SAVE_MOA_PRESETS, presets) as Promise<
      MoAPreset[]
    >,
  /** 列出 CLI 工人配置（首次调用主进程就地 seed 默认：总开关 enabled=false，零行为变化） */
  listCliWorkersConfig: () =>
    ipcRenderer.invoke(
      AGENT_IPC_CHANNELS.LIST_CLI_WORKERS,
    ) as Promise<CliWorkersConfig>,
  /**
  ...
  probeCliWorkers: (cfg?: CliWorkersConfig) =>
    ipcRenderer.invoke(
      ...
      cfg,
    ) as Promise<CliWorkersProbeResult>,
  /** 读 No-Progress Guard 模式（effective / stored / envOverride） */
  getNoProgressGuardMode: () =>
    ipcRenderer.invoke(
      AGENT_IPC_CHANNELS.GET_NO_PROGRESS_GUARD_MODE,
    ) as Promise<{
  ...
  killSessionProcess: (sessionId: string, id: string) =>
    ipcRenderer.invoke(AGENT_IPC_CHANNELS.KILL_SESSION_PROCESS, {
      sessionId,
      id,
    }),
  onSessionProcessesChanged: (
    cb: (payload: { sessionId: string; processes: unknown[] }) => void,
  ) => {
    const handler = (
      _e: unknown,
      payload: { sessionId: string; processes: unknown[] },
    ): void => cb(payload);
  ...
  onStreamEvent: (cb: (payload: unknown) => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(AGENT_IPC_CHANNELS.STREAM_EVENT, handler);
  },

  // ===== 渠道 =====
  /** 列出所有渠道（apiKey 加密，不泄露明文） */
  listChannels: () =>
    ipcRenderer.invoke(CHANNEL_IPC_CHANNELS.LIST) as Promise<Channel[]>,
  ...
  updateChannel: (id: string, patch: ChannelUpdateInput) =>
    ipcRenderer.invoke(CHANNEL_IPC_CHANNELS.UPDATE, { id, patch }) as Promise<
      Channel | undefined
    >,
  /** 删除渠道（kscc-internal 不可删） */
  deleteChannel: (id: string) =>
    ipcRenderer.invoke(CHANNEL_IPC_CHANNELS.DELETE, id) as Promise<{
      ok: boolean;
      error?: string;
    }>,
  /** 测试渠道连接 */
  ...
  /** 列出所有工作区 */
  listWorkspaces: () =>
    ipcRenderer.invoke(AGENT_IPC_CHANNELS.LIST_WORKSPACES) as Promise<
      AgentWorkspace[]
    >,
  /** 创建项目工作区（弹出文件夹选择对话框） */
  createProjectWorkspace: () =>
    ...
    ) as Promise<AgentWorkspace | null>,
  ...
  deleteMcpServer: (slug: string, name: string) =>
    ipcRenderer.invoke(AGENT_IPC_CHANNELS.SAVE_MCP_CONFIG + ":delete", {
      slug,
      name,
    }) as Promise<{ ok: boolean; error?: string }>,
  /** 真实测试 MCP server 连接（成功/失败顺带持久化 lastTestResult） */
  testMcpServer: (slug: string, name: string, entry: McpServerEntry) =>
    ipcRenderer.invoke(AGENT_IPC_CHANNELS.TEST_MCP_SERVER, {
      slug,
      name,
      entry,
  ...
  onPermissionRequest: (cb: (req: unknown) => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(
        AGENT_IPC_CHANNELS.PERMISSION_REQUEST,
        handler,
      );
  },
  ...
  onPermissionResolved: (cb: (payload: unknown) => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(
        AGENT_IPC_CHANNELS.PERMISSION_RESOLVED,
        handler,
      );
  },
  respondToPermission: (
    ...
    ipcRenderer.send(AGENT_IPC_CHANNELS.PERMISSION_RESPOND, {
      reqId,
      behavior,
      remember,
    }),
  ...
  onAskUserRequest: (cb: (request: AskUserRequest) => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(AGENT_IPC_CHANNELS.ASK_USER_REQUEST, handler);
  },
  ...
  onAskUserResolved: (cb: (e: { requestId: string }) => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(AGENT_IPC_CHANNELS.ASK_USER_RESOLVED, handler);
  },
  ...
  onExitPlanModeRequest: (cb: (request: ExitPlanModeRequest) => void) => {
    const handler = (_e: unknown, request: ExitPlanModeRequest): void =>
      cb(request);
    ipcRenderer.on(AGENT_IPC_CHANNELS.EXIT_PLAN_MODE_REQUEST, handler);
    return () =>
      ipcRenderer.removeListener(
        AGENT_IPC_CHANNELS.EXIT_PLAN_MODE_REQUEST,
        handler,
      );
  },
  ...
  onPlanModeChanged: (
    ...
    return () =>
      ipcRenderer.removeListener(AGENT_IPC_CHANNELS.PLAN_MODE_CHANGED, handler);
  },
  // 热切换指定会话的权限模式（持久化 meta + 通知运行时）
  setSessionPermissionMode: (sessionId: string, mode: string) =>
    ipcRenderer.invoke(AGENT_IPC_CHANNELS.UPDATE_SESSION_PERMISSION_MODE, {
      sessionId,
      mode,
    }) as Promise<{ ok: boolean; error?: string }>,
  ...
  // ===== 角色库 =====
  listAgentRoles: () =>
    ipcRenderer.invoke(AGENT_ROLE_IPC_CHANNELS.LIST) as Promise<
      AgentRoleProfile[]
    >,
  getAgentRole: (roleId: string) =>
    ipcRenderer.invoke(AGENT_ROLE_IPC_CHANNELS.GET, roleId) as Promise<
      AgentRoleProfile | undefined
    >,
  saveAgentRole: (input: SaveAgentRoleInput) =>
    ipcRenderer.invoke(AGENT_ROLE_IPC_CHANNELS.SAVE, input) as Promise<
      AgentRoleProfile[]
    >,
  ...
  resetDefaultAgentRoles: () =>
    ipcRenderer.invoke(AGENT_ROLE_IPC_CHANNELS.RESET_DEFAULT) as Promise<
      AgentRoleProfile[]
    >,
  listRoleStoreCatalog: () =>
    ipcRenderer.invoke(
      AGENT_ROLE_IPC_CHANNELS.STORE_LIST,
    ) as Promise<RoleStoreCatalogResult>,
  installStoreRole: (roleId: string) =>
    ipcRenderer.invoke(
      AGENT_ROLE_IPC_CHANNELS.STORE_INSTALL,
      roleId,
    ) as Promise<InstallStoreRoleResult>,
  importAgentRoleFromMd: () =>
    ipcRenderer.invoke(
      AGENT_ROLE_IPC_CHANNELS.IMPORT_MD,
    ) as Promise<ImportRoleFromMdResult>,
  findSimilarAgentRoles: (displayName: string) =>
    ipcRenderer.invoke(
      AGENT_ROLE_IPC_CHANNELS.FIND_SIMILAR,
      displayName,
    ) as Promise<AgentRoleProfile[]>,
  deleteAgentRolesBatch: (roleIds: string[]) =>
    ipcRenderer.invoke(
      AGENT_ROLE_IPC_CHANNELS.DELETE_BATCH,
      roleIds,
    ) as Promise<DeleteRolesResult>,
  // ===== Bot 库 =====
  listBots: () =>
    ipcRenderer.invoke(BOT_IPC_CHANNELS.LIST) as Promise<BotProfileRecord[]>,
  getBot: (profileId: string) =>
    ipcRenderer.invoke(BOT_IPC_CHANNELS.GET, profileId) as Promise<
      BotProfileRecord | undefined
    >,
  saveBot: (input: SaveBotProfileInput) =>
    ipcRenderer.invoke(BOT_IPC_CHANNELS.SAVE, input) as Promise<
      BotProfileRecord[]
    >,
  createBot: (input: CreateBotProfileInput) =>
    ipcRenderer.invoke(
      BOT_IPC_CHANNELS.CREATE,
      input,
    ) as Promise<BotProfileRecord>,
  archiveBot: (profileId: string) =>
    ipcRenderer.invoke(
      BOT_IPC_CHANNELS.ARCHIVE,
      profileId,
    ) as Promise<BotProfileRecord>,
  publishBotConfigRevision: (input: PublishBotConfigRevisionInput) =>
    ipcRenderer.invoke(
      BOT_IPC_CHANNELS.PUBLISH_REVISION,
      input,
    ) as Promise<BotProfileRecord>,
  listBotMemories: (profileId: string) =>
    ipcRenderer.invoke(BOT_IPC_CHANNELS.MEMORY_LIST, profileId) as Promise<
      BotMemoryRecord[]
    >,
  saveBotMemoryCandidate: (input: SaveBotMemoryCandidateInput) =>
    ipcRenderer.invoke(
      BOT_IPC_CHANNELS.MEMORY_SAVE_CANDIDATE,
      input,
    ) as Promise<BotMemoryRecord>,
  activateBotMemory: (memoryId: string) =>
    ipcRenderer.invoke(
      BOT_IPC_CHANNELS.MEMORY_ACTIVATE,
      memoryId,
    ) as Promise<BotMemoryRecord>,
  rejectBotMemory: (memoryId: string) =>
    ipcRenderer.invoke(
      BOT_IPC_CHANNELS.MEMORY_REJECT,
      memoryId,
    ) as Promise<BotMemoryRecord>,
  archiveBotMemory: (memoryId: string) =>
    ipcRenderer.invoke(
      BOT_IPC_CHANNELS.MEMORY_ARCHIVE,
      memoryId,
    ) as Promise<BotMemoryRecord>,
  consolidateBotMemory: (input: ConsolidateBotMemoryInput) =>
    ipcRenderer.invoke(
      BOT_IPC_CHANNELS.MEMORY_CONSOLIDATE,
      input,
    ) as Promise<BotMemoryConsolidationResult>,
  /** 打开/恢复当前会话中的 Bot 旁路悬浮窗 */
  openBotSidecar: (input: OpenBotSidecarInput) =>
    ipcRenderer.invoke(
      BOT_IPC_CHANNELS.SIDECAR_OPEN,
      input,
    ) as Promise<BotSidecarState>,
  ensureBotSidecarSession: (input: EnsureBotSidecarSessionInput) =>
    ipcRenderer.invoke(
      BOT_IPC_CHANNELS.SIDECAR_SESSION_ENSURE,
      input,
    ) as Promise<string>,
  /** 关闭 Bot 旁路悬浮窗，主会话与 Bot 长期配置不受影响 */
  closeBotSidecar: (input: CloseBotSidecarInput) =>
    ipcRenderer.invoke(
      BOT_IPC_CHANNELS.SIDECAR_CLOSE,
      input,
    ) as Promise<BotSidecarState | null>,
  /** 收起为吸边球 */
  minimizeBotSidecar: (sidecarId: string) =>
    ipcRenderer.invoke(
      BOT_IPC_CHANNELS.SIDECAR_MINIMIZE,
      sidecarId,
    ) as Promise<BotSidecarState | null>,
  /** 旁路 Bot 请求把已确认内容交给主会话 Agent */
  requestBotSidecarBridge: (input: BotSidecarBridgeRequest) =>
    ipcRenderer.invoke(
  ...
  onExecutionModeSuggestion: (cb: (suggestion: unknown) => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(
        AGENT_IPC_CHANNELS.EXECUTION_MODE_SUGGESTION,
        handler,
      );
  },
  /** 关闭建议条（不切换 mode） */
  ...
  kanbanResumeBoard: (boardId: string, sessionId?: string) =>
    ipcRenderer.invoke(KANBAN_IPC_CHANNELS.RESUME_BOARD, {
      boardId,
      sessionId,
    }),
  /** 解除阻塞（blocked → ready） */
  ...
  kanbanCommentTask: (taskId: string, comment: string, author = "main") =>
    ipcRenderer.invoke(KANBAN_IPC_CHANNELS.COMMENT_TASK, {
      taskId,
      comment,
      author,
    }),
  onKanbanChanged: (cb: (payload: unknown) => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(KANBAN_IPC_CHANNELS.CHANGED, handler);
  },
  onKanbanBoardCompleted: (cb: (payload: unknown) => void) => {
    const handler = (_e: unknown, payload: unknown): void => cb(payload);
    ipcRenderer.on(KANBAN_IPC_CHANNELS.BOARD_COMPLETED, handler);
    return () =>
      ipcRenderer.removeListener(KANBAN_IPC_CHANNELS.BOARD_COMPLETED, handler);
  },

  /** 手动压缩会话上下文（Pi 核；需会话已在本机启动） */
  compactSession: (sessionId: string) =>
    ipcRenderer.invoke(AGENT_IPC_CHANNELS.COMPACT_SESSION, {
      sessionId,
    }) as Promise<{
      ok: boolean;
      compacted: boolean;
   ...
   */
  getSystemDark: () =>
    ipcRenderer.invoke("theme:get-system-dark") as Promise<boolean>,
   ...
   */
  setResolvedDark: (dark: boolean) =>
    ipcRenderer.send("theme:set-resolved-dark", dark),
  ...
  onNudgeEvent: (cb: (payload: unknown) => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(MEMORY_IPC_CHANNELS.NUdge_EVENT, handler);
  },
  getStageQueue: (mode: "general" | "ta") =>
    ipcRenderer.invoke(MEMORY_IPC_CHANNELS.GET_STAGE_QUEUE, mode) as Promise<
      unknown[]
    >,
  acceptStageAll: (mode: "general" | "ta") =>
    ipcRenderer.invoke(MEMORY_IPC_CHANNELS.ACCEPT_STAGE_ALL, mode) as Promise<
      unknown[]
    >,
  rejectStageAll: (mode: "general" | "ta") =>
    ipcRenderer.invoke(MEMORY_IPC_CHANNELS.REJECT_STAGE_ALL, mode) as Promise<
      unknown[]
    >,
  acceptStageOne: (mode: "general" | "ta", id: string) =>
    ipcRenderer.invoke(MEMORY_IPC_CHANNELS.ACCEPT_STAGE_ONE, {
      mode,
      id,
    }) as Promise<{
      ok: boolean;
  ...
  rejectStageOne: (mode: "general" | "ta", id: string) =>
    ipcRenderer.invoke(MEMORY_IPC_CHANNELS.REJECT_STAGE_ONE, {
      mode,
      id,
    }) as Promise<{
      ok: boolean;
  ...
  getChannelBalance: (channelId: string) =>
    ipcRenderer.invoke(
      ...
      channelId,
    ) as Promise<ChannelBalanceResult>,

  // ===== 协作室（Stage 1：房间壳 + 静态成员 + 静态消息，不运行 Agent） =====
  /** 列出全部协作室房间（默认不含已归档） */
  listCollaborationRooms: (input?: { includeArchived?: boolean }) =>
    ipcRenderer.invoke(COLLABORATION_ROOM_IPC_CHANNELS.LIST, input) as Promise<
      CollaborationRoom[]
    >,
  /** 创建协作室房间（含可选静态成员） */
  ...
  /** 将当前融合会话升级为协作室；原会话保留，升级操作幂等 */
  upgradeFusionSessionToRoom: (input: UpgradeFusionSessionInput) =>
    ipcRenderer.invoke(
  ...
  getCollaborationRoom: (roomId: string) =>
    ipcRenderer.invoke(COLLABORATION_ROOM_IPC_CHANNELS.GET, {
      roomId,
    }) as Promise<CollaborationRoom | null>,
  ...
  joinCollaborationHumanMember: (input: JoinCollaborationHumanMemberInput) =>
    ipcRenderer.invoke(
      COLLABORATION_ROOM_IPC_CHANNELS.JOIN_HUMAN_MEMBER,
      input,
    ) as Promise<CollaborationHumanMember>,
  leaveCollaborationHumanMember: (input: LeaveCollaborationHumanMemberInput) =>
    ipcRenderer.invoke(
  ...
  /** 软删除成员（保留历史与 Bot 加入时快照） */
  removeCollaborationMember: (input: RemoveCollaborationMemberInput) =>
    ipcRenderer.invoke(
  ...
  listCollaborationMemberPresets: () =>
    ...
    ) as Promise<CollaborationMemberPreset[]>,
  ...
  listCollaborationMailbox: (roomId: string) =>
    ipcRenderer.invoke(COLLABORATION_ROOM_IPC_CHANNELS.LIST_MAILBOX, {
      roomId,
    }) as Promise<CollaborationMailboxEnvelope[]>,
  /**
  ...
  listCollaborationRoomTasks: (roomId: string) =>
    ipcRenderer.invoke(COLLABORATION_ROOM_IPC_CHANNELS.LIST_ROOM_TASKS, {
      roomId,
    }) as Promise<CollaborationRoomTask[]>,
  ...
  listCollaborationArtifacts: (roomId: string) =>
    ipcRenderer.invoke(COLLABORATION_ROOM_IPC_CHANNELS.LIST_ARTIFACTS, {
      roomId,
    }) as Promise<CollaborationArtifact[]>,
  /**
  ...
  downloadCollaborationArtifact: (input: {
    ...
    ipcRenderer.invoke(
      ...
      input,
    ) as Promise<DownloadCollaborationArtifactResult>,
  ...
  listCollaborationUserApprovals: (roomId: string) =>
    ipcRenderer.invoke(COLLABORATION_ROOM_IPC_CHANNELS.LIST_USER_APPROVALS, {
      roomId,
    }) as Promise<CollaborationUserApprovalRequest[]>,
  /** 批准或拒绝成员的用户审批请求。 */
  ...
  onCollaborationRoomChanged: (
    ...
    const handler = (
      _e: unknown,
      payload: { roomId: string; kind: string; at: number },
    ): void => cb(payload);
    ipcRenderer.on(COLLABORATION_ROOM_IPC_CHANNELS.CHANGED, handler);
    return () =>
      ipcRenderer.removeListener(
        COLLABORATION_ROOM_IPC_CHANNELS.CHANGED,
        handler,
      );
  },
  /** 成员 turn 流式正文增量（不走 CHANGED，避免每 token 全量刷新） */
  onCollaborationTextDelta: (
    cb: (payload: CollaborationTextDeltaPayload) => void,
  ) => {
    const handler = (
      _e: unknown,
    ...
    return () =>
      ipcRenderer.removeListener(
        COLLABORATION_ROOM_IPC_CHANNELS.TEXT_DELTA,
        handler,
      );
  },
  ...
  browserSetBounds: (input: BrowserSetBoundsRequest) =>
    ipcRenderer.invoke(
      BROWSER_IPC_CHANNELS.SET_BOUNDS,
      input,
    ) as Promise<BrowserWorkspaceState>,
  browserHide: (sessionId: string) =>
    ipcRenderer.invoke(BROWSER_IPC_CHANNELS.HIDE, sessionId) as Promise<{
      ok: boolean;
    }>,
  ...
  browserNewTab: (input: BrowserTabRequest) =>
    ipcRenderer.invoke(
      BROWSER_IPC_CHANNELS.NEW_TAB,
      input,
    ) as Promise<BrowserWorkspaceState>,
  browserCloseTab: (input: BrowserTabRequest) =>
    ipcRenderer.invoke(
      BROWSER_IPC_CHANNELS.CLOSE_TAB,
      input,
    ) as Promise<BrowserWorkspaceState>,
  browserSelectTab: (input: BrowserTabRequest) =>
    ipcRenderer.invoke(
      BROWSER_IPC_CHANNELS.SELECT_TAB,
      input,
    ) as Promise<BrowserWorkspaceState>,
  ...
  browserType: (input: BrowserElementActionRequest) =>
    ipcRenderer.invoke(BROWSER_IPC_CHANNELS.TYPE, input) as Promise<{
      ok: boolean;
      message?: string;
    }>,
  browserScroll: (input: BrowserScrollRequest) =>
    ipcRenderer.invoke(BROWSER_IPC_CHANNELS.SCROLL, input) as Promise<{
      ok: boolean;
      message?: string;
    }>,
  browserScreenshot: (sessionId: string) =>
    ipcRenderer.invoke(
      BROWSER_IPC_CHANNELS.SCREENSHOT,
      sessionId,
    ) as Promise<BrowserScreenshotResult>,
  browserStop: (sessionId: string) =>
    ipcRenderer.invoke(BROWSER_IPC_CHANNELS.STOP, sessionId) as Promise<{
      ok: boolean;
      message?: string;
    }>,
  browserTakeover: (input: BrowserTakeoverRequest) =>
    ipcRenderer.invoke(
      BROWSER_IPC_CHANNELS.TAKEOVER,
      input,
    ) as Promise<BrowserWorkspaceState>,
  browserResume: (sessionId: string) =>
    ipcRenderer.invoke(
      BROWSER_IPC_CHANNELS.RESUME,
      sessionId,
    ) as Promise<BrowserWorkspaceState>,
  onBrowserStateChanged: (cb: (state: BrowserWorkspaceState) => void) => {
    const handler = (_e: unknown, state: BrowserWorkspaceState): void =>
      cb(state);
    ipcRenderer.on(BROWSER_IPC_CHANNELS.STATE_CHANGED, handler);
    return () =>
      ipcRenderer.removeListener(BROWSER_IPC_CHANNELS.STATE_CHANGED, handler);
  },
  onBrowserOpenRequest: (cb: (request: BrowserOpenRequest) => void) => {
    const handler = (_e: unknown, request: BrowserOpenRequest): void =>
      cb(request);
  ...
  updater: {
    ...
    installWhenIdle: () =>
      ipcRenderer.invoke("updater:install-when-idle") as Promise<boolean>,
    cancelIdleInstall: () =>
      ipcRenderer.invoke("updater:cancel-idle-install") as Promise<void>,
    onStatusChanged: (cb: (status: unknown) => void) => {
      ...
      return () =>
        ipcRenderer.removeListener("updater:on-status-changed", handler);
    },
  },
} as const;
