 ...
 */
const NON_SESSION_PANE_PREFIXES = [
  "crew:",
  "file-preview:",
  "mermaid-preview:",
  "rich-preview:",
...
function resolveTabInsertIndex(
  ...
  if (i === dest.size - 1 && e.nativeEvent?.clientX != null) {
    ...
    const tabEl = t instanceof Element ? t.closest?.(".dv-tab") : null;
    if (tabEl) {
...
function handleWillDrop(e: {
  ...
  if (
    (e.kind === "tab" || e.kind === "header_space") &&
    data?.panelId &&
    dest
  ) {
    e.preventDefault();
...
export function WorkspaceDock(): JSX.Element {
  ...
  useEffect(() => {
    ...
    root
      .querySelectorAll("[data-dock-active-plate]")
    ...
    try {
      ...
      if (toAdopt.length > 0) {
        ...
        rejected.forEach((tab) => {
          if (!sessionRunMap[tab.sessionId]?.running)
            api.getPanel(tab.sessionId)?.api.close();
        });
      }
  ...
  useEffect(() => {
    const off = browserApi().onBrowserOpenRequest((request) =>
  ...
  const handleReady = (e: { api: DockviewApi }): void => {
    ...
    if (fromLayout.length > 0) {
      ...
      rejected.forEach((tab) => {
        if (!sessionRunMap[tab.sessionId]?.running)
          e.api.getPanel(tab.sessionId)?.api.close();
      });
    }
  ...
  return (
    <div ref={rootRef} className="workspace-dock h-full min-h-0">
      <div className="workspace-dock__summary app-workspace-tab-actions titlebar-no-drag">
        {showBotRailAction ? (
          <AppTooltip
