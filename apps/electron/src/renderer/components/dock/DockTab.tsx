...
export function DockTab(props: IDockviewPanelHeaderProps<DockTabParams>): JSX.Element {
  ...
  return (
    <div className="app-workspace-tab-shell group relative h-full w-full" data-active={active || undefined}>
      <button
        type="button"
        onClick={() => api.setActive?.()}
        className="app-workspace-tab titlebar-no-drag h-full w-full"
      >
        <Icon size={14} weight="regular" className="app-workspace-tab__icon shrink-0" />
        <span className="app-workspace-tab__title">{api.title || '会话'}</span>
      </button>
      <button
        type="button"
        onClick={(e) => {
