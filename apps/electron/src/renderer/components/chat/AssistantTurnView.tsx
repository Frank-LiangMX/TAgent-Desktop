...
interface AssistantTurnViewProps {
  ...
  /** result 没有正文时的显式状态：等待后台回调，或已收口但缺少最终答复。 */
...
export function AssistantTurnView({
  ...
  pinnedSubagentIdsRef.current = pinnedSubagentIdsRef.current.filter(
    (id) => !nestedIds.has(id),
  );
  ...
  const subagentEntryIds = pinnedSubagentIdsRef.current;
  ...
  const needsTypewriter =
    ...
    (presentation.isStreaming ||
      ...
      (isLiveTurn &&
        Boolean(content) &&
        Boolean(streamText || presentation.isStreaming)) ||
  ...
  const turnCreatedAt = mainItems.find((it) => it.message?.type === "assistant")
    ?.message?.createdAt;
  ...
  const completionMs =
    !isLiveTurn && completedDuration ? completedDuration.ms : 0;
  ...
  return (
    <div className="agent-turn flex flex-col" data-speaker={speakerName}>
      ...
      {subagentEntryIds.length > 0 ? (
        <div className="agent-concise-subagent-dock">
          {renderTurnSubagents(
            subagentEntryIds,
            turn.items,
            subagentCards,
          ...
          <Message from="assistant">
            <MessageContent>
              {displayedContent.trim() ? (
                ...
                <MessageResponse streaming={needsTypewriter}>
                  {displayedContent}
                </MessageResponse>
              ) : processLive ? (
                <MessageLoading />
...
function TurnEndFooter({
  ...
  const tooltip = [label, duration ? `用时 ${duration}` : undefined, clock]
    .filter(Boolean)
    .join(" · ");
  return (
    <AppTooltip label={tooltip}>
      ...
      >
        <Icon
          className="agent-turn-outcome__icon"
          size={13}
          weight="bold"
          aria-hidden
        />
        ...
        {duration ? (
          <span className="agent-turn-outcome__detail">{duration}</span>
        ) : null}
        {clockTime ? (
          <span className="agent-turn-outcome__clock">{clockTime}</span>
        ) : null}
      </span>
    </AppTooltip>
  );
