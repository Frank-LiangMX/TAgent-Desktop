...
describe("streamState：delta 不修改 items（验收 2 / 防 R3）", () => {
  ...
  it("turn key 在仅有 streamState、无流式占位 item 时由 synthetic turn 稳定", () => {
    const items = [
      {
        key: "u1",
        message: { type: "user", content: [{ type: "text", text: "hi" }] },
      },
    ];
...
describe("clearSessionStreamState", () => {
  it("清空 text/thinking", () => {
    const dirty = applyTextDelta(
      applyThinkingDeltaToState(EMPTY_STREAM_STATE, "x"),
      "y",
    );
...
describe("tool-only sdk_message 不得清 thinking（防出完即消失）", () => {
  ...
  it("stop_reason 但 content 无 thinking → 保留 stream thinking（REGRESS-E）", () => {
    let state = applyThinkingDeltaToState(
      EMPTY_STREAM_STATE,
      "完整思考不应被剥",
    );
    state = applySdkMessageToStreamState(state, {
...
describe("applySdkMessageToItems：uuid 原地 upsert（S2.1 单真源）", () => {
  ...
  it("不同 uuid 的消息各占一条（partial→final→下一轮 partial 不覆盖上一轮 final）", () => {
    ...
    expect(
      items[0]?.message?.type === "assistant"
        ? items[0].message.uuid
        : undefined,
    ).toBe("u1");
    expect(
      items[1]?.message?.type === "assistant"
        ? items[1].message.uuid
        : undefined,
    ).toBe("u2");
    expect(items[0]?.streaming).toBe(false);
  ...
  it("首条 thinking + tool 落盘快照不吞思考", () => {
    const stripped = {
      ...
      content: [
        { type: "thinking", thinking: "" },
        { type: "tool_use", id: "t-first", name: "Read", input: {} },
      ],
    } as TAgentMessage;

    // 与 renderer 事件顺序一致：先 upsert 快照，再提交同帧 thinking。
    let items = applySdkMessageToItems<StreamItemLike>([], stripped, alloc);
    items = commitStreamThinkingToLastAssistant(items, "首条思考");
    const m = items[0]?.message;
    expect(m?.type).toBe("assistant");
    if (m?.type === "assistant") {
  ...
  it("commitStreamThinking 末条已有思考时仍可落到更早的空壳（多段思考不丢）", () => {
    ...
    if (m0?.type === "assistant") {
      expect(m0.content[0]).toMatchObject({
        type: "thinking",
        thinking: "第二段思考应填到 u1 空壳",
      });
    }
    const m1 = items[1]?.message;
    if (m1?.type === "assistant") {
      expect(m1.content[0]).toMatchObject({
        type: "thinking",
        thinking: "第一段思考",
      });
    }
  ...
  it("工具终态无 text：shouldClearStreamText 为真 → 须先 commit 再清（main 无 tool_start）", () => {
    ...
    if (m?.type === "assistant") {
      expect(
        m.content.some(
          (b) => b.type === "text" && (b as { text: string }).text === pending,
        ),
      ).toBe(true);
    }
    // 清缓冲后 presentation 仍能从 message text 投出 narrative
    const cleared = applySdkMessageToStreamState(
      { text: pending, thinking: "想" },
      toolFinal,
    );
