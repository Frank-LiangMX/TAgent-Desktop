 ...
 */
export function groupSubagentItems(
  items: TurnSourceItem[],
): TurnSourceItem[][] {
...
export function listSubagentEntryIds(items: TurnSourceItem[]): string[] {
  ...
  for (const it of items) {
    ...
    const card = it.taskCard as
      { toolUseId?: string; taskType?: string } | undefined;
...
export function classifyRunAbort(input: {
  ...
  const { userStopped, lastUserStopAt, lastAskUserDismissAt, now, errorText } =
    input;
  const abortLike =
    userStopped ||
  ...
  if (
    ...
    now - lastAskUserDismissAt < RUN_ASK_DISMISS_WINDOW_MS
  ) {
    return "complete";
  }
...
export function sliceItemsBeforeLastRealUser<T extends TurnSourceItem>(
  items: T[],
): T[] | null {
  for (let i = items.length - 1; i >= 0; i--) {
    const m = items[i]?.message;
...
export function getLastMainAssistantCreatedAt(
  items: TurnSourceItem[],
): number | undefined {
  for (let i = items.length - 1; i >= 0; i--) {
    const m = items[i]?.message;
    if (m?.type === "assistant" && !m.parentToolUseId && m.createdAt)
      return m.createdAt;
  }
  return undefined;
}
...
function getTurnLastMainAssistantCreatedAt(
  items: TurnSourceItem[],
): number | undefined {
...
export function isRealUserInput(message: TAgentUserMessage): boolean {
  ...
  return message.content.some(
    (b) =>
      b.type === "text" &&
      typeof (b as TAgentTextBlock).text === "string" &&
      (b as TAgentTextBlock).text.trim().length > 0,
  );
}
...
export function isCrewNoticeMessage(message: TAgentAssistantMessage): boolean {
  ...
  for (const b of message.content) {
    if (b.type === "text" && typeof (b as TAgentTextBlock).text === "string") {
      if ((b as TAgentTextBlock).text.trimStart().startsWith("【班组完成】"))
        return true;
    }
  }
  return false;
...
export function groupItemsIntoTurns(
  ...
  let current: Extract<SessionRenderTurn, { kind: "assistant-turn" }> | null =
    null;
  ...
  for (const item of items) {
    ...
    if (
      ...
      const mainlineModel =
        msg?.type === "assistant" && !msg.parentToolUseId
          ? msg.modelId
          : undefined;
      if (!current) {
        current = {
...
export function areToolsBeforeIndexCompleted(
  ...
  for (let index = 0; index < endIndex; index++) {
    ...
    if (!completedToolResultIds.has((block as TAgentToolUseBlock).id))
      return false;
  }
  return true;
}
...
export function buildTurnPresentation(
  ...
  for (const item of turn.items) {
    ...
    if (item.streaming) {
      ...
      if (item.streamingThinking != null)
        streamingThinking = item.streamingThinking;
    }
  ...
  if (isLiveTurn && externalStream) {
    streamingText = externalStream.text
      ...
      : undefined;
    streamingThinking = externalStream.thinking
      ? externalStream.thinking
      : undefined;
  ...
  const toolUseSeen = new Map<
    string,
    { block: TAgentToolUseBlock; key: string; rich: boolean }
  >();
  for (const item of turn.items) {
    ...
    const rich = message.content.some(
      (b) => b.type === "thinking" || b.type === "text",
    );
    const ownerKey = message.uuid ?? item.key;
    const at = message.createdAt;
    const fullThinking = (
      message as TAgentMessage & { __fullThinking?: unknown }
    ).__fullThinking;
    const fullThinkingText =
      typeof fullThinking === "string" ? fullThinking.trim() : "";
    let usedFullThinking = false;
    message.content.forEach((block, blockIndex) => {
      if (block.type === "tool_use") {
        ...
        if (prev) {
          ...
          if (rich && !prev.rich) {
            const idx = timeline.findIndex(
              (x) => x.kind === "block" && x.key === prev.key,
            );
            if (idx >= 0)
              timeline[idx] = { kind: "block", block, key: prev.key, at };
        ...
        const displayBlock =
          fullThinkingText && !usedFullThinking
            ? { ...block, thinking: fullThinkingText }
            : block;
        usedFullThinking = true;
        timeline.push({
          kind: "block",
          block: displayBlock,
          key: "think-" + ownerKey + "-" + blockIndex,
          at,
        });
  ...
  const allBlocks = timeline.filter(
    (entry): entry is Extract<TimelineSource, { kind: "block" }> =>
      entry.kind === "block",
  );
  ...
  if (
    allBlocks.some(
      (x) =>
        x.block.type === "thinking" &&
        (x.block as TAgentThinkingBlock).thinking.trim(),
    ) &&
    turn.items.some(
  ...
  const lastFinalAssistantText = (() => {
    for (let i = turn.items.length - 1; i >= 0; i--) {
      ...
      if (
        ...
        for (const b of m.content) {
          if (b.type === "text" && (b as TAgentTextBlock).text.trim())
            txt = (b as TAgentTextBlock).text;
        }
        return txt;
      }
    }
  ...
  if (
    allBlocks.some(
      (x) =>
        x.block.type === "text" && (x.block as TAgentTextBlock).text.trim(),
    ) &&
    turn.items.some(
      (it) => it.message?.type === "assistant" && isFinalAssistant(it.message),
    ) &&
  ...
  // 运行中的 assistant 文本仍可能是阶段性思路/进度，即使前面的工具已经完成。
  // 只有收到终态 assistant（或整轮结束后 isLiveTurn=false）才允许把尾部文本外置到回答壳；
  // 否则 full 模式会把长思考链提前当成最终回答，concise 模式也失去统一的过程入口。
  ...
  const canShowAnswerWhileLive = !isLiveTurn || hasFinalAssistant;
  ...
  for (const entry of timeline) {
    ...
    if (block.type === "thinking") {
      ...
      const text = sanitizeAssistantTextForDisplay(
        (block as TAgentTextBlock).text,
      );
  ...
  if (
    ...
    const guidance = process.filter(
      (p): p is Extract<ProcessEntry, { type: "guidance" }> =>
        p.type === "guidance",
    );
    process.length = 0;
  ...
  if ((isStreaming || isLiveTurn) && streamingThinking?.trim()) {
    ...
    if (lastThinkIdx >= 0) {
      const last = process[lastThinkIdx] as Extract<
        ProcessEntry,
        { type: "thinking" }
      >;
      ...
      if (
        ...
        process[lastThinkIdx] = {
          type: "thinking",
          key: last.key,
          thinking: streamingThinking,
        };
      } else if (
        !process.some((p) => p.type === "thinking" && p.thinking.trim() === st)
      ) {
        process.push({
          type: "thinking",
          key: "stream-thinking",
  ...
  if (holdStreamInProcess && streamText) {
    ...
    if (lastTextIdx >= 0) {
      const last = process[lastTextIdx] as Extract<
        ProcessEntry,
        { type: "text" }
      >;
      if (
  ...
  const answerOverlay = (() => {
    ...
    if (streamText && answerJoined) {
      if (
        streamText.startsWith(answerJoined) ||
        answerJoined.startsWith(streamText)
      ) {
        return streamText.length >= answerJoined.length
          ? streamText
          : answerJoined;
      }
      return streamText;
  ...
  const keepStream =
    ...
    !(
      answerJoined &&
      (answerJoined === streamText || answerJoined.startsWith(streamText))
    );
...
export function formatThinkingSummary(
  ...
  if (durationSec != null && durationSec >= 3)
    return `思考了 ${formatElapsedDuration(durationSec * 1000)}`;
  return "思考了片刻";
}
