...
import { loadChannelsAtom } from "./atoms/channel-atoms";
import {
  workspacesAtom,
  loadWorkspacesAtom,
...
declare global {
  interface Window {
    electronAPI: {
      ...
      saveCliWorkersConfig: (
        cfg: CliWorkersConfig,
      ) => Promise<CliWorkersConfig>;
      /** 本机探测 CLI 工人是否可用 */
      probeCliWorkers: (
        cfg?: CliWorkersConfig,
      ) => Promise<CliWorkersProbeResult>;
      ...
      setAgentDiscussPrefs: (
        prefs: AgentDiscussPrefs,
      ) => Promise<AgentDiscussPrefs>;
      ...
      fetchModelsForChannel: (
        input: FetchModelsForChannelInput,
      ...
      getSessionStatus: (
        id: string,
      ) => Promise<{ status: "idle" | "running" | "error"; archived: boolean }>;
      ...
      getInstalledPluginBundles: (
        slug: string,
      ) => Promise<WorkspacePluginBundleRecord[]>;
      ...
      onExitPlanModeRequest: (
        cb: (request: ExitPlanModeRequest) => void,
      ) => () => void;
      ...
      onExecutionModeSuggestion: (
        cb: (suggestion: unknown) => void,
      ) => () => void;
      dismissExecutionModeSuggestion: (
        sessionId: string,
      ) => Promise<{ ok: boolean }>;
      ...
      // 角色库
      listAgentRoles: () => Promise<
        import("@tagent/shared").AgentRoleProfile[]
      >;
      getAgentRole: (
        roleId: string,
      ) => Promise<import("@tagent/shared").AgentRoleProfile | undefined>;
      saveAgentRole: (
      ...
      resetDefaultAgentRoles: () => Promise<
        import("@tagent/shared").AgentRoleProfile[]
      >;
      listRoleStoreCatalog: () => Promise<
        import("@tagent/shared").RoleStoreCatalogResult
      >;
      installStoreRole: (
        roleId: string,
      ) => Promise<import("@tagent/shared").InstallStoreRoleResult>;
      importAgentRoleFromMd: () => Promise<
        import("@tagent/shared").ImportRoleFromMdResult
      >;
      findSimilarAgentRoles: (
        displayName: string,
      ) => Promise<import("@tagent/shared").AgentRoleProfile[]>;
      deleteAgentRolesBatch: (
        roleIds: string[],
      ) => Promise<import("@tagent/shared").DeleteRolesResult>;
      // Bot 库
      listBots: () => Promise<import("@tagent/shared").BotProfileRecord[]>;
      getBot: (
        profileId: string,
      ) => Promise<import("@tagent/shared").BotProfileRecord | undefined>;
      saveBot: (
        input: import("@tagent/shared").SaveBotProfileInput,
      ) => Promise<import("@tagent/shared").BotProfileRecord[]>;
      createBot: (
        input: import("@tagent/shared").CreateBotProfileInput,
      ) => Promise<import("@tagent/shared").BotProfileRecord>;
      archiveBot: (
        profileId: string,
      ) => Promise<import("@tagent/shared").BotProfileRecord>;
      publishBotConfigRevision: (
        input: import("@tagent/shared").PublishBotConfigRevisionInput,
      ) => Promise<import("@tagent/shared").BotProfileRecord>;
      listBotMemories: (
        profileId: string,
      ) => Promise<import("@tagent/shared").BotMemoryRecord[]>;
      saveBotMemoryCandidate: (
        input: import("@tagent/shared").SaveBotMemoryCandidateInput,
      ) => Promise<import("@tagent/shared").BotMemoryRecord>;
      activateBotMemory: (
        memoryId: string,
      ) => Promise<import("@tagent/shared").BotMemoryRecord>;
      ...
      consolidateBotMemory: (
        input: import("@tagent/shared").ConsolidateBotMemoryInput,
      ) => Promise<import("@tagent/shared").BotMemoryConsolidationResult>;
      openBotSidecar: (
        input: import("@tagent/shared").OpenBotSidecarInput,
      ) => Promise<import("@tagent/shared").BotSidecarState>;
      ensureBotSidecarSession: (
        input: import("@tagent/shared").EnsureBotSidecarSessionInput,
      ) => Promise<string>;
      closeBotSidecar: (
        input: import("@tagent/shared").CloseBotSidecarInput,
      ) => Promise<import("@tagent/shared").BotSidecarState | null>;
      minimizeBotSidecar: (
        sidecarId: string,
      ) => Promise<import("@tagent/shared").BotSidecarState | null>;
      requestBotSidecarBridge: (
        input: import("@tagent/shared").BotSidecarBridgeRequest,
      ...
      removeCollaborationHumanMember: (input: {
        roomId: string;
        userId: string;
        actorUserId?: string;
      }) => Promise<CollaborationHumanMember>;
      setCollaborationBotOwnerConsent: (input: {
        roomId: string;
        memberId: string;
        consent: boolean;
        actorUserId?: string;
      ...
      listCollaborationMemberPresets: () => Promise<
        CollaborationMemberPreset[]
      >;
      ...
      listCollaborationRoomTasks: (
        roomId: string,
      ) => Promise<CollaborationRoomTask[]>;
      createCollaborationRoomTask: (
...
export function App(): JSX.Element {
  ...
  useEffect(() => {
    const handleOpenCollaborationRoom = (): void => {
      // 协作已经是当前会话的内部模式；升级后仍停留在会话页。
      setActiveRail("chat");
      setSidebarOpen(true);
    };
    window.addEventListener(
      "tagent:open-collaboration-room",
      handleOpenCollaborationRoom,
    );
    return () => {
  ...
  useEffect(() => {
    ...
    setActiveTabId((current) =>
      current && retainedIds.has(current)
        ? current
        : (result.tabs.at(-1)?.id ?? null),
    );
    // 同步关掉被逐出的 Dockview pane，防止其被布局恢复逻辑再次收编进 tabsAtom。
    result.evictedTabs.forEach((tab) =>
    ...
    void (async () => {
      try {
        ...
        if (!boardId && input.taskId) {
          const task = (await window.electronAPI.kanbanGetTask?.(
            input.taskId,
          )) as { boardId?: string } | null | undefined;
        ...
        const sessions =
          (await window.electronAPI.listSessions()) as Array<SessionMeta>;
        ...
        pushTicker(
          makeStatusTickerItem("打开班组失败，请稍后重试", "error", 4500),
        );
      }
    })();
  };
  ...
  useEffect(() => {
    const action = (input: {
      ...
      coalesceKey: input.taskId
        ? `kanban-task:${input.taskId}`
        : `kanban-board:${input.boardId ?? ""}`,
    });
    const off1 = window.electronAPI.onKanbanChanged?.((payload: unknown) => {
      ...
      if (p.status === "running") {
        pushTicker(
          makeStatusTickerItem("班组：任务开始执行", "info", 4000, action(p)),
        );
      } else if (p.status === "done") {
        pushTicker(
          makeStatusTickerItem("班组：任务已完成", "success", 5000, action(p)),
        );
      } else if (p.status === "failed") {
        pushTicker(
          makeStatusTickerItem("班组：任务失败", "error", 7000, action(p)),
        );
  ...
  return (
    <TooltipProvider delayDuration={280} skipDelayDuration={120}>
      <RichSourceContext.Provider value={{ resolve: richSourceResolver }}>
        ...
        />
        <Dialog
          open={tabCapacityDialogOpen}
          onOpenChange={setTabCapacityDialogOpen}
        >
          <DialogContent
            className="w-[min(380px,calc(100vw-32px))] gap-5 p-5 sm:max-w-none"
            hideClose
          >
            <DialogHeader className="space-y-2 text-left">
              ...
              <DialogDescription className="text-[12.5px] leading-5">
                最多可打开 {MAX_SESSION_TABS}{" "}
                个会话。请等待任一会话完成或停止后，再打开新的会话。
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
