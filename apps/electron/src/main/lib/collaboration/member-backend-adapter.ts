 ...
 *   其余走 Pi HTTP 直连。
 * - 成员未绑定 channelId：取第一个 enabled 外部（非 kscc）渠道 + 其默认模型；
 *   无则抛 MemberBackendResolveError('NO_EXTERNAL_CHANNEL', '请先配置外部渠道…')。
 *
...
export function resolveChannelBackendConfig(input: {
  ...
  if (input.channelId) {
    ...
    if (!channel) {
      throw new MemberBackendResolveError(
        'NO_CHANNEL',
        `成员绑定的渠道已删除（id=${input.channelId}），请重新配置成员后端`,
      )
    }
    if (!channel.enabled) {
      throw new MemberBackendResolveError(
    ...
    const modelId = resolveModelId(channel, input.modelId)
    if (!modelId) {
      throw new MemberBackendResolveError(
        'NO_MODEL',
  ...
  // 2) 未绑定：取第一个 enabled 外部（非 kscc）渠道
  const external = listChannels().filter((c) => c.enabled && c.provider !== 'kscc-internal')
  ...
  throw new MemberBackendResolveError(
    'NO_EXTERNAL_CHANNEL',
    '没有可用的外部渠道：请在设置 → 渠道中配置并启用一个非 kscc 渠道（含 API Key 与模型）',
  )
}
