...
interface BrowserDownloadCapture {
  filePath: string;
...
function browserWindowIconPath(): string {
  ...
  const resourcesDir = existsSync(fromDist)
    ? fromDist
  ...
  return path.join(
    ...
    "logo",
    "appicon",
    nativeTheme.shouldUseDarkColors ? "dark.png" : "light.png",
  );
}
...
export class BrowserController {
  ...
  private readonly activeDownloadCaptures = new Map<
    string,
    { tempDirectory: string }
  >();
  private readonly pendingDownloadCaptures = new Map<
    string,
  ...
  private activeTab(sessionId: string): BrowserTabRuntime {
    ...
    const tab = workspace.activeTabId
      ? workspace.tabs.get(workspace.activeTabId)
      : undefined;
  ...
  private publish(workspace: BrowserWorkspaceRuntime): BrowserWorkspaceState {
    const state: BrowserWorkspaceState = {
      ...
      ...(workspace.takeoverReason
        ? { takeoverReason: workspace.takeoverReason }
        : {}),
    };
    const win = this.getWindow();
    if (win && !win.isDestroyed())
      win.webContents.send(BROWSER_IPC_CHANNELS.STATE_CHANGED, state);
    return state;
  }

  private attachTab(
    workspace: BrowserWorkspaceRuntime,
  ...
  private async extractFromContents(
    ...
    for (const frame of frames) {
      try {
        ...
        )) as { text?: string; score?: number; title?: string };
        if (result?.text?.trim())
          results.push({
            text: result.text,
  ...
  async openDetached(
    ...
    url: string,
    title?: string,
  ): Promise<BrowserDetachedWindowState> {
    ...
    if (!detached || detached.isDestroyed()) {
      detached = new BrowserWindow({
        ...
        ...(existsSync(browserWindowIconPath())
          ? { icon: browserWindowIconPath() }
          : {}),
      ...
      detached.webContents.session.setPermissionRequestHandler(
        (_webContents, _permission, callback) => callback(false),
      );
      ...
      detached.on("closed", () => {
        if (this.detachedWindows.get(sessionId) === detached)
          this.detachedWindows.delete(sessionId);
      });
    ...
    if (!detached.isDestroyed()) {
      this.prepareDownloadCapture(sessionId);
      detached.show();
      detached.focus();
    }
    return {
      ...
      url: detached.isDestroyed()
        ? normalizedUrl
        : detached.webContents.getURL() || normalizedUrl,
      title: detached.isDestroyed()
        ? browserWindowTitle(title)
        : detached.getTitle() || browserWindowTitle(title),
    };
  }
  ...
  async extractDetachedText(sessionId: string): Promise<BrowserPageTextResult> {
    ...
    if (!detached || detached.isDestroyed())
      throw new Error("独立浏览器窗口已关闭，请重新打开云文档");
    return this.extractFromContents(
  ...
  private webContentsForCapture(sessionId: string): WebContents | null {
    const detached = this.detachedWindows.get(sessionId);
    if (detached && !detached.isDestroyed()) return detached.webContents;
  ...
  async captureDownload(
    ...
    const pending = this.pendingDownloadCaptures.get(sessionId);
    if (pending) return pending;
    const wc = this.webContentsForCapture(sessionId);
    if (!wc || wc.isDestroyed()) {
      throw new Error("内置浏览器未打开，请先打开云文档并完成登录");
    }
    const partitionSession = wc.session;
    if (this.activeDownloadCaptures.has(sessionId)) {
      throw new Error("已有一次下载捕获在等待，请先完成或取消后再试");
    }
    const timeoutMs = Math.max(
      10000,
      Math.min(900000, options.timeoutMs ?? 600000),
    );
    const tempDirectory = mkdtempSync(
      path.join(tmpdir(), "tagent-kb-download-"),
    );
    partitionSession.setDownloadPath(tempDirectory);
    let resolved = false;
    const capture = new Promise<BrowserDownloadCapture>((resolve, reject) => {
      const finish = (action: () => void): void => {
        ...
        partitionSession.off("will-download", onWillDownload);
        this.activeDownloadCaptures.delete(sessionId);
        this.pendingDownloadCaptures.delete(sessionId);
        action();
      };
      ...
      poller = setInterval(() => {
        try {
          const candidates = readdirSync(tempDirectory, { withFileTypes: true })
            .filter(
              (entry) =>
                entry.isFile() &&
          ...
          if (candidate && candidate.stat.size > 0) {
            if (
              candidate.filePath === lastCandidate &&
              candidate.stat.size === lastSize
            )
              stablePolls += 1;
            else {
              lastCandidate = candidate.filePath;
          ...
          // Download directory may be cleaned while the capture is finishing.
        }
      }, 500);
      const timer = setTimeout(() => {
        finish(() => {
          ...
          reject(
            new Error("等待下载超时，请在内置浏览器中点击「下载为 DOCX/XLSX」"),
          );
        });
      }, timeoutMs);
      const onWillDownload = (
        ...
        const filename = item.getFilename() || "document";
        const mimeType = item.getMimeType() || "";
        const extension = path.extname(filename).toLowerCase();
        const tempFile = path.join(
          tempDirectory,
          extension ? filename : filename + ".bin",
        );
        ...
        item.once("done", (_event, state) => {
          if (state === "completed") {
            ...
            finish(() => {
              rmSync(tempDirectory, { recursive: true, force: true });
              reject(new Error("下载未完成：" + state));
            });
          }
        });
      };
      this.activeDownloadCaptures.set(sessionId, { tempDirectory });
      partitionSession.on("will-download", onWillDownload);
    });
    this.pendingDownloadCaptures.set(sessionId, capture);
    return capture;
  }

  prepareDownloadCapture(sessionId: string): BrowserActionResult {
    void this.captureDownload(sessionId).catch(() => undefined);
    return action(true, "Download capture is ready");
  }
  ...
  hide(sessionId: string): BrowserActionResult {
    for (const tab of this.workspace(sessionId).tabs.values())
  ...
  selectTab(input: BrowserTabRequest): BrowserWorkspaceState {
    ...
    if (!tab) return this.publish(workspace);
    for (const candidate of workspace.tabs.values())
  ...
  async observe(sessionId: string): Promise<BrowserObserveResult> {
    ...
    const elements = (await tab.view.webContents.executeJavaScript(
      script,
  ...
  async click(
    input: BrowserElementActionRequest,
  ): Promise<BrowserActionResult> {
    const tab = this.activeTab(input.sessionId);
    this.validateRef(tab, input.ref);
    const script =
      '(function () { const node = document.querySelector("[data-tagent-ref=" + ' +
      JSON.stringify(input.ref) +
      ' + "]"); if (!node) return false; node.scrollIntoView({ block: "center", inline: "nearest" }); node.click(); return true; })()';
    const clicked = await tab.view.webContents.executeJavaScript(script);
    return clicked
      ? action(true, "已点击页面元素")
      : action(false, "找不到页面元素，请重新观察页面");
  }
  ...
  async type(input: BrowserElementActionRequest): Promise<BrowserActionResult> {
    ...
    const script =
      '(function () { const node = document.querySelector("[data-tagent-ref=" + ' +
      JSON.stringify(input.ref) +
      ' + "]"); if (!node) return false; node.focus(); const value = ' +
      JSON.stringify(text) +
      '; if (node.isContentEditable) node.textContent = value; else { const setter = Object.getOwnPropertyDescriptor(node.__proto__, "value")?.set; if (setter) setter.call(node, value); else node.value = value; } node.dispatchEvent(new Event("input", { bubbles: true })); node.dispatchEvent(new Event("change", { bubbles: true }));' +
      enter +
      " return true; })()";
    const typed = await tab.view.webContents.executeJavaScript(script);
    return typed
      ? action(true, "已填写页面元素")
      : action(false, "找不到可填写元素，请重新观察页面");
  }
  ...
  async screenshot(sessionId: string): Promise<BrowserScreenshotResult> {
    ...
    const scale = Math.min(
      1,
      1280 / Math.max(size.width, 1),
      900 / Math.max(size.height, 1),
    );
    const output =
      scale < 1
  ...
  takeover(sessionId: string, reason?: string): BrowserWorkspaceState {
    ...
    workspace.takeoverReason =
      reason || "请在浏览器标签中完成登录、验证码或其他人工步骤";
    return this.publish(workspace);
  }
  ...
  dispose(): void {
    ...
    for (const capture of this.activeDownloadCaptures.values()) {
      try {
        rmSync(capture.tempDirectory, { recursive: true, force: true });
      } catch {
        /* ignore */
      }
    }
    this.activeDownloadCaptures.clear();
    this.pendingDownloadCaptures.clear();
  }
}
