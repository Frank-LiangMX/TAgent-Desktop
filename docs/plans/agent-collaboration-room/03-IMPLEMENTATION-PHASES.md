...
- P0-2 本地 fixture E2E 已落地（2026-08-23）：`apps/electron/src/main/lib/collaboration/fusion-room-multiuser-fixture.test.ts` 用 loopback HTTP·SSE 跑通双用户 / 双 Bot owner / 共享工作区 / 邀请 token 身份（含 `principal.roomId` → `SCOPE_MISMATCH`）/ Bot owner consent / 费用主体归属 / 可下载产物 + SSE 加分项，仅新增测试、未改源码。真实账户 / 证书 / 跨机器 / 打包公网入口仍未做，见 [12-IMPLEMENTATION-LOG §73](./12-IMPLEMENTATION-LOG-2026-08-22.md)。
- P0-3 证书 + 打包版显式闸门已落地（2026-08-23）：`fusion-room-cert-store.ts` 交付 RoomSession TLS 证书本地生命周期（generate/load/rotate/revoke，纯 `node:crypto` 自签，无外部 CA 库）；`fusion-room-network-prefs.ts` 交付纯函数 `decidePackagedCollaborationGate` + 持久化偏好；`main/index.ts` 已用该函数替代硬编码 `!app.isPackaged`，**默认 prefs 全关 → 打包行为不变**（协作室 IPC 不注册、非 loopback 不监听），`enableNetworkListen` 还需 active 证书；`allowInsecureNetwork` 等明文公网开关被显式拒绝。真实账户 / 生产 CA / 远程 transport 从 Electron 启动接通 `allowNonLoopbackListen` / 设置页危险区 UI 仍未做，见 [12-IMPLEMENTATION-LOG §74](./12-IMPLEMENTATION-LOG-2026-08-22.md)。
- P0-3b 设置页危险区 UI + 闸门状态接线已落地（2026-08-23）：设置页「关于」底部挂「融合会话 · 协作与网络（危险区）」——`enableCollaboration` / `enableNetworkListen` 开关（后者在 `!enableCollaboration` 或 `!hasActiveCert` 时 disabled）+ 证书列表 / 生成 / 撤销 + 只读闸门状态卡（含待重启徽章）；新增 `fusion-room-network-prefs:gate-status` IPC（`getFusionRoomGateStatus` 经 preload + App.tsx 暴露）。**应用策略选 B**：闸门变更提示重启生效，不动态注册 / 卸载 IPC；**默认仍全关、不自动起非 loopback 监听**；未做实机点击手测（无 Electron GUI），以组件模型单测 + typecheck 验证，见 [12-IMPLEMENTATION-LOG §75](./12-IMPLEMENTATION-LOG-2026-08-22.md)。
- P0-3c transport 启动接通已落地（2026-08-23）：`fusion-room-transport-bootstrap.ts` 交付 `bootstrapFusionRoomTransport`——据显式闸门 + cert store 的 active TLS 材料决定是否 `createFusionRoomTransportRuntime` 并 `start`（`allowNonLoopbackListen` + TLS → HTTPS 非 loopback `listening`；无 TLS → `failed` 不得明文非 loopback；`!allowNonLoopbackListen` → `disabled`，可选 `enableDevLoopbackTransport` 仅强制 127.0.0.1 明文）；`main/index.ts` 删 `console.log` 占位改真实 bootstrap + 日志 + `before-quit` close；新增 `fusion-room-transport:status` 只读 IPC（`appliedGate` + transport 投影）+ preload / App.tsx `getFusionRoomTransportStatus` + 设置页一行只读「传输：…」。`enableDefaultMemberExecution` 始终 false、fallback 认证 deny-all。**默认仍关**；未做真实账户 OAuth / 跨机器实机 E2E / 热插拔 listen / 实机 GUI 手测，见 [12-IMPLEMENTATION-LOG §83](./12-IMPLEMENTATION-LOG-2026-08-22.md)。
...
- 未完成且不能宣传为已完成：真实账户认证、打包版真实公网监听接通与跨机器多用户 E2E、跨节点单写者/事件总线/预算原子预留、真实 Pi/KSCC/provider 生命周期验收、持久 continuation worker、Git worktree、完整远程操作 UI、分页虚拟化和 memory consolidation E2E。
