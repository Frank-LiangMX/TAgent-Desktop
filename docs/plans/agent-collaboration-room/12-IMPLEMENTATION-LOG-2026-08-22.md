...
## 91. 单会话升级后的工作区继承与 Agent 上下文（2026-08-25）

修复“单会话升级为协作室后 Agent 找不到原项目”的工作区断链：

- 升级房间继续保存 sourceSessionId + workspaceId；解析成员工作区时，来源会话房间优先回到原项目目录，不再被新建的空 roomWorkspace 抢占。
- 来源项目目录失效时 fail-closed，不回退到空协作目录，避免 Agent 在错误目录中读写。
- 每个成员 turn 的 system prompt 现在明确注入宿主解析出的工作区根目录、工作区类型和相对路径规则；独立协作室仍提示其隔离工作区与个人目录的区别。
- 来源会话房间头部显示“已连接来源会话的项目工作区”，不再显示容易误导的“导入个人工作区”入口；独立房间继续保留显式导入。
- 类型验证：bun run typecheck 全部通过。
- 本轮未进行实机 GUI 测试，未运行测试套件；保留既有 Electron 测试环境边界记录。


## 92. 协作室工作区绑定可视化（2026-08-25）

- 共享契约新增工作区绑定视图：每条绑定包含用户显示名、工作区类型、workspaceId、可用状态和宿主目录。
- 人类成员可记录自己的 workspaceId；来源单会话升级自动把房主绑定到来源项目工作区，后续远程成员可在接受邀请时声明自己的工作区。
- 新增只读 IPC 查询工作区绑定，渲染层不直接解析路径，也不把目录写入共享房间实体。
- 协作室标题栏新增紧凑“工作区”Popover：默认只显示数量，展开后按用户展示完整目录；房间共享目录单独标注，长路径使用截断 + title 查看。
- 设计约束：不在顶部常驻铺开长路径，避免再次造成信息堆叠；目录归属不依赖颜色单独表达。
- 验证：bun run typecheck 全部通过；未进行实机 GUI 测试。
