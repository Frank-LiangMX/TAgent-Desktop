...
## [2.0.0-dev.6] - 2026-08-31

本次更新聚焦于协作会话、自动化任务、Agent 执行稳定性、记忆质量和桌面工作台体验。完整对外文案见 [release-notes/v2.0.0-dev.6.md](./release-notes/v2.0.0-dev.6.md)。

### 新增
- **Fusion 协作流程** — 完善协作房间、成员会话、任务执行和结果回传流程，支持用户确认后继续、运行中取消和失败状态处理。
- **自动化任务** — 增加自动化任务的创建、编辑、启停、调度和运行状态展示。
- **记忆质量与检索** — 增加记忆候选质量判断，改进记忆范围、相关性排序、证据收集和空闲整理。

### 体验
- 协作工作台、成员状态、运行进度、结果摘要和待处理任务展示更清晰。
- 会话启动、进程复用、历史恢复、浏览器控制和附件进入 Agent 上下文的过程更稳定。
- 消息时间线、阶段状态、分屏、侧栏、启动状态和会话路由进一步收束。

### 修复
- 修复协作任务取消、自动化调度、后台运行和异常退出后的状态清理问题。
- 修复长会话历史恢复、流式消息收尾、任务结果展示和部分 IPC 状态回传问题。
- 修复记忆候选重复、范围判断、检索结果以及会话切换、滚动和界面布局问题。

## [2.0.0-dev.5] - 2026-08-20

相对 `dev.4`：聚焦记忆沉淀、分屏工作台、长会话体验、任务运行状态和消息展示细节。完整对外文案见 [release-notes/v2.0.0-dev.5.md](./release-notes/v2.0.0-dev.5.md)。

### 新增
- **频繁切换会话提醒分屏** — 检测到多个会话频繁切换时提示开启分屏，并支持确认后自动左右分屏。
- **记忆沉淀链路增强** — 完善记忆证据收集、空闲整理、会话落盘和持久化调度。
- **受限浏览器** — Agent 可以在独立的浏览器标签页中打开网页、阅读页面、点击、输入和滚动；遇到登录或验证码时，用户可以接管操作，完成后继续任务。
- **本机工具与环境兼容性提升** — 改进本机辅助工具的能力识别和路径处理，提升不同安装环境下的可用性。
- **长时间任务后台运行** — 长时间任务可以继续在后台运行，并在当前会话中查看运行状态。

### 体验
- 发送新一轮消息时预留流式输出空间，历史补页保持滚动位置，滚动刻度避让运行计时和输入区域。
- 中文 Markdown 标点展示、文件路径芯片、分屏 Dock、会话状态和长会话导航进一步优化。
- 较长的思考过程会整理和折叠，任务结果明确区分完成、中断、出错、等待后续任务和缺少最终答复。
- 长会话按对话轮次加载历史，简洁时间线展开后保留阶段细节和实时状态。

### 修复
- 修复流式消息被输入框遮挡、首次落盘思考消息被吞、滚动到底判断异常和文件路径芯片下行字母被裁切等问题。
- 修复子任务入口重复展示、长思考内容重复堆叠和任务结束后显示空壳完成状态的问题。

## [2.0.0-dev.4] - 2026-08-19

相对 `dev.3`：会话工作台、Agent 执行稳定性、消息展示和文件工作流体验得到进一步改进。完整对外文案见 [release-notes/v2.0.0-dev.4.md](./release-notes/v2.0.0-dev.4.md)。

### 新增
- **会话时间线与摘要** — 优化轮次模型、运行状态、会话摘要和文件变更审阅。
- **无进展检测** — Agent 长时间重复尝试或没有有效推进时提示用户介入。
- **附件与 HTML 预览** — 图片附件保留在历史中，HTML 文件支持安全预览。

### 体验
- 长会话滚动、运行中引导、停止操作和后台进程状态更稳定。
- 消息区域、侧栏信息、滚动导航、标题栏状态和 Tooltip 进一步打磨。
- 子代理入口、会话状态和文件路径展示更加清晰。

### 修复
- 修复消息发送跳回上一轮、完成折叠动画、文件预览和会话状态恢复等问题。

## [2.0.0-dev.3] - 2026-08-13

相对 `dev.2`：会话工作台与顶部通知中心完成一轮交互打磨；会诊席位、后台进程和消息队列更可见；自动记忆整理在打包版恢复工作。完整对外文案见 [release-notes/v2.0.0-dev.3.md](./release-notes/v2.0.0-dev.3.md)。

### 新增
- **顶部通知中心** — 完成与运行中会话通知、班组入口、展开查看与一键清除。
- **会话工作台** — 4 标签上限、运行中保护、响应式标签压缩与更稳定的分屏切换。
- **会诊状态** — 席位按设置动态生成，展示等待 / 运行 / 完成 / 失败状态。

### 体验
- 正文字号与行间距独立预览；字号档位、刻度及标签对齐。
- 引导、队列、完成信息、Tooltip 和运行状态动画统一收束。
- 自动记忆整理使用空闲调度；仅 KSCC 时可使用 DeepSeek V4 Flash。

### 修复
- 关闭标签/分屏后的留白与标签底板残影。
- 计时显示、实时引导持久化、会诊席位配置和记忆图谱把会话误标为 Skill。
