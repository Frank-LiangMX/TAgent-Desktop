...
const seats = [
  { id: 'seat_a', status: 'idle' as const, isCoordinator: false, createdAt: 10 },
  { id: 'seat_b', status: 'idle' as const, isCoordinator: true, createdAt: 20 },
  { id: 'seat_c', status: 'paused' as const, isCoordinator: false, createdAt: 30 },
]
...
describe('fusion routing', () => {
  ...
  test('只有一个可用 Bot 时就是普通直聊路径', () => {
    expect(
      resolveFusionRoute({ seats: seats.map((seat) => ({ ...seat, status: 'paused' as const }))
  ...
  test('没有可用席位时不静默回退到普通 Agent', () => {
    expect(resolveFusionRoute({ seats: seats.map((seat) => ({ ...seat, status: 'removed' as const })) })).toEqual({
      ok: false,
