...
function mkMember(id: string, displayName: string, isCoordinator = false): CollaborationMember {
  return {
    id,
    roomId: 'cr_x',
    displayName,
...
describe('projectCollaborationTurnContext（S3.5-a H2，04 §5.4）', () => {
  test('C1 自己历史发言 → role=assistant，无 [自己名]: 前缀', () => {
    ...
    const trigger = mkMsg('m2', { authorType: 'user', content: '继续', createdAt: 2 })
  ...
  test('C2 其他成员发言 → user + [显示名]: 前缀', () => {
    ...
    const t = project({ member: me, members: [me, other], messages: [msg], trigger: msg })
  ...
  test('C3 用户发言 → user + [用户]: 前缀', () => {
    const me = mkMember('cm_a', '分析师')
    const msg = mkMsg('m1', { authorType: 'user', authorId: 'user', content: '帮我查一下', createdAt: 1 })
  ...
  test('C4 正文含 @协调者 → 投影后不保留该 token', () => {
    ...
    const t = project({ member: me, members: [me, coord], messages: [msg], trigger: msg })
  ...
  test('C5 user_only → 任何成员都看不到', () => {
    ...
    const t = project({ member: me, messages: [msg], trigger: msg })
    expect(t.messages.some((m) => m.content.includes('只给系统看的'))).toBe(false)
  })
  ...
  test('C6 participants 定向给 B → A 看不到；B 与协调者看得到', () => {
    ...
    const forA = project({ member: a, members: [coord, a, b], messages: [msg], trigger: msg })
    expect(forA.messages.some((m) => m.content.includes('这份约束'))).toBe(true) // 作者可见

    const forB = project({ member: b, members: [coord, a, b], messages: [msg], trigger: msg })
    expect(forB.messages.some((m) => m.content.includes('这份约束'))).toBe(true) // 目标可见

    const triggerForA = mkMsg('m9', { authorType: 'user', content: '继续', createdAt: 9 })
    ...
    expect(forOther.messages.some((m) => m.content.includes('这份约束'))).toBe(false)
  })

  test('C7 摘要存在 → 最先注入，带「二级信息」声明', () => {
    const me = mkMember('cm_a', '分析师')
    const msg = mkMsg('m1', { authorType: 'user', content: '继续', createdAt: 1 })
    const t = project({
      member: me,
      messages: [msg],
      trigger: msg,
  ...
  test('C8 代码围栏内 @开发 → 保留', () => {
    ...
    const t = project({ member: me, members: [me, dev], messages: [msg], trigger: msg })
  ...
  test('C9 continuation → 末尾含勿重复副作用', () => {
    ...
    const t = project({ member: me, members: [me, b], messages: [reply], trigger: reply })
    const last = t.messages[t.messages.length - 1]!
    expect(last.content).toContain('A2A 恢复')
    expect(last.content).toContain('不要重复已经做过的副作用操作')
    // reply 正文不重复出现两次
    expect(t.messages.filter((m) => m.content.includes('接口约束已确认'))).toHaveLength(1)
  })
