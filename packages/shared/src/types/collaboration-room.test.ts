...
describe("validateCreateCollaborationRoomInput", () => {
  ...
  test("title 为空报错", () => {
    expect(validateCreateCollaborationRoomInput({ title: "" })).toContain(
      "title 不能为空",
    );
    expect(validateCreateCollaborationRoomInput({ title: "   " })).toContain(
      "title 不能为空",
  ...
  test("title 超长报错", () => {
    ...
    expect(validateCreateCollaborationRoomInput(input)).toContain(
      "title 长度不能超过 200",
    );
  });
  ...
  test("成员超过上限报错", () => {
    const members = Array.from(
      { length: COLLABORATION_ROOM_MAX_MEMBERS + 1 },
      () => ({
        displayName: "成员",
      }),
    );
  ...
  test("成员 displayName 为空报错", () => {
    ...
    const errors = validateCreateCollaborationRoomInput(input);
    expect(
      errors.some((e) => e.includes("members[0].displayName 不能为空")),
    ).toBe(true);
  ...
  test("maxA2ADepth 超过硬上限报错", () => {
    expect(
      validateCreateCollaborationRoomInput({
        title: "t",
        maxA2ADepth: COLLABORATION_ROOM_HARD_MAX_A2A_DEPTH + 1,
      }),
...
describe("parseCollaborationMentions", () => {
  ...
  test("@displayName 精确命中（忽略大小写）", () => {
    ...
    expect(parseCollaborationMentions("@开发 做点事", members)).toEqual([
      "cm_dev",
    ]);
    expect(
      parseCollaborationMentions("@协调者 @开发 两人都来", members),
    ).toEqual(["cm_coord", "cm_dev"]);
  });

  test("英文 displayName 忽略大小写匹配", () => {
    const members = [mkMember("cm_a", "Alice"), mkMember("cm_b", "Bob")];
    expect(parseCollaborationMentions("@alice @BOB", members)).toEqual([
      "cm_a",
      "cm_b",
    ]);
  });
  ...
  test("@all 与具体点名共存：去重后仍为全部成员", () => {
    ...
    expect(parseCollaborationMentions("@all @开发", members)).toEqual([
      "cm_coord",
      "cm_dev",
    ]);
  });
  ...
  test("无匹配的 @ 忽略（不报错、不影响其他命中）", () => {
    ...
    expect(parseCollaborationMentions("@不存在 @开发", members)).toEqual([
      "cm_dev",
    ]);
  });
  ...
  test("末尾标点被剥掉（@开发。 → 开发）", () => {
    ...
    expect(parseCollaborationMentions("@开发, @开发；", members)).toEqual([
      "cm_dev",
    ]);
  });
  ...
  test("同一成员多次 @ 仅记录一次", () => {
    const members = [mkMember("cm_dev", "开发")];
    expect(parseCollaborationMentions("@开发 @开发 @开发", members)).toEqual([
      "cm_dev",
    ]);
  });
  ...
  test("改名后 @旧名 仍命中 mentionAliases", () => {
    const members = [mkMember("cm_dev", "主程")];
    members[0]!.mentionAliases = ["开发"];
    expect(parseCollaborationMentions("@开发 看下", members)).toEqual([
      "cm_dev",
    ]);
    expect(parseCollaborationMentions("@主程 看下", members)).toEqual([
      "cm_dev",
    ]);
  });
  ...
  test("当前 displayName 占用同名时优先于别人的别名", () => {
    ...
    expect(parseCollaborationMentions("@开发", [oldDev, newDev])).toEqual([
      "cm_new",
    ]);
  });
  ...
  test("@memberId 精确命中", () => {
    const members = [mkMember("cm_dev", "开发")];
    expect(parseCollaborationMentions("@cm_dev 看下", members)).toEqual([
      "cm_dev",
    ]);
  });
});
...
describe("resolveCollaborationMentions（S3.5-a H1，04 §4.5）", () => {
  ...
  test("M3 结构化 {memberId:开发}，正文写 @协调者 → 只开发（正文忽略）", () => {
    const r = resolve("@协调者 你好", {
      structured: [
        { kind: "agent", memberId: "cm_dev", displayNameSnapshot: "开发" },
      ],
    });
    expect(r.targetMemberIds).toEqual(["cm_dev"]);
  });
  ...
  test("M7 引用块内 @开发 不命中", () => {
    expect(
      resolve(
        '<quoted_message message_id="m1" author="用户">@开发</quoted_message> 请看',
      ).targetMemberIds,
    ).toEqual([]);
  });
  ...
  test("M15 已移除成员不再被文本、结构化或 @all 路由命中", () => {
    const all = members();
    all[1] = { ...all[1]!, status: "removed", removedAt: 10 };
    const text = resolveCollaborationMentions({
      text: "@开发 @all",
      members: all,
      sender: { type: "user" },
    });
    expect(text.targetMemberIds).toEqual(["cm_coord", "cm_qa"]);
    const structured = resolveCollaborationMentions({
      text: "@开发",
      members: all,
      structured: [{ kind: "agent", memberId: "cm_dev" }],
      sender: { type: "user" },
    });
    expect(structured.targetMemberIds).toEqual([]);
    expect(structured.dropped).toEqual([
      { token: "cm_dev", reason: "unknown-member-id" },
    ]);
  });
});
...
describe("nextCollaborationMentionAliases", () => {
  test("旧名进入别名，新名从别名摘掉", () => {
    expect(nextCollaborationMentionAliases(["前端"], "开发", "主程")).toEqual([
      "前端",
      "开发",
    ]);
    expect(
      nextCollaborationMentionAliases(["主程", "开发"], "主程", "开发"),
    ).toEqual(["主程"]);
  });
...
describe("validateCollaborationRoomBudget", () => {
  test("接受有效预算，拒绝非整数和越界值", () => {
    ...
    expect(
      validateCollaborationRoomBudget({ maxUsageTokens: 1.5 }),
    ).not.toEqual([]);
  });
});

describe("CLI worker 成员校验", () => {
  test("要求 worker 与 workspace-write", () => {
    const base: CreateCollaborationRoomInput = {
      title: "CLI room",
      members: [{ displayName: "开发", backend: "cli" }],
    };
    expect(validateCreateCollaborationRoomInput(base)).toContain(
      "members[0].cliWorkerId 不能为空",
    );
    expect(
      validateCreateCollaborationRoomInput({
        ...base,
    ...
    ).toContain("members[0] 使用 CLI worker 时必须是 workspace-write");
    expect(
      validateCreateCollaborationRoomInput({
        ...base,
        members: [
